...
EnterNextPrestige(autoCalled)
{
	...
	if(self.pers["rank"] == level.maxRank && self.pers["rankxp"] >= maxrankxp && self.pers["prestige"] != level.maxPrestige)
	{
		...
		self setstat( 2356, self.pers["rank"] );
		self setstat( 2357, self.pers["prestige"] );

		self setRank( 0, self.pers["prestige"] );
		self setStat( 2359, self getStat(2359) + 1 );
	}
	else
	{
		if(!autoCalled)
		{
			if(self.pers["prestige"] == level.maxPrestige)
				iPrintlnBold("PRESTIGE_MAX_REACHED", self);
			else
			{
				iPrintlnBold("PRESTIGE_REQUIREMENTS", self);
				
				if(self.pers["rank"] != level.maxRank)
					self iPrintLnBold("You have to reach rank " + (level.maxRank+1) + " first.");
				else if(self.pers["rankxp"] < maxrankxp)
					self iPrintLnBold("You need " + (int(level.rankTable[self.pers["prestige"]][level.maxRank][7]) - self.pers["rankxp"]) + " more XP.");
			}
		}
	}	
}
...
UpdateRankInfo()
{
	...
	self maps\mp\gametypes\_persistence::statSet( "minxp", int(level.rankTable[self.pers["rank"]][2])); // min xp for the rank
	self maps\mp\gametypes\_persistence::statSet( "maxxp", int(level.rankTable[self.pers["rank"]][7])); // max xp for the rank
}